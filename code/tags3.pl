...
sub leveler {
    ...
    while ( $objKey ) {
        ...
        # Checking status of levelReffArray
...
sub populate {
    ...
    if ( $objKey ne 'TOP' && exists $data->{ matches }->{$objKey} ) {
        divyMatches( $data );
    }
    else {
...
sub mes {
    ...
    my $indent = "  ";
    my $lvl = 0;
    if ( exists $data->{point} ) { $lvl = scalar $data->{point}->@* ?  scalar $data->{point}->@* : 0; }
